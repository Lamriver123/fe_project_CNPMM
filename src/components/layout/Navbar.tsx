// src/components/Navbar.tsx
import React from "react";
import { useNavigate } from "react-router-dom";
import "./Navbar.css";

const Navbar: React.FC = () => {
  const navigate = useNavigate();

  return (
    <nav className="uts-navbar">
      <div className="uts-container">
        {/* Logo */}
        <a className="uts-logo" href="/">
          <img src="data:image/png;base64," alt="logo" className="uts-logo-img" />
          <span className="uts-logo-text">UTEShop</span>
        </a>

        {/* Menu */}
        <ul className="uts-navlinks">
          <li><a href="/home">Trang chủ</a></li>
          <li><a href="/products">Sản phẩm</a></li>
          <li><a href="/about">Giới thiệu</a></li>
          <li><a href="/contact">Liên hệ</a></li>
        </ul>

        {/* Search bar */}
        <div className="uts-search">
          <input
            type="text"
            placeholder="Tìm kiếm sản phẩm, danh mục hoặc thương hiệu..."
          />
          <button>
            <i className="bi bi-search"></i>
          </button>
        </div>

        {/* Actions */}
        <div className="uts-actions">
          {/* Wishlist */}
          <button className="uts-icon-btn">
            <i className="bi bi-heart"></i>
          </button>

          {/* Cart */}
          <button className="uts-icon-btn position-relative">
            <i className="bi bi-cart"></i>
            <span className="uts-badge">2</span>
          </button>

          {/* Auth buttons */}
          <button
            className="uts-btn uts-btn-login"
            onClick={() => navigate("/login")}
          >
            Đăng nhập
          </button>
          <button
            className="uts-btn uts-btn-signup"
            onClick={() => navigate("/register")}
          >
            Đăng ký
          </button>
        </div>
      </div>
    </nav>
  );
};

export default Navbar;
